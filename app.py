...
# ══════════════════════════════════════════════════════════════════════════════
# DESIGN SYSTEM  — Emote Finance aesthetic
# Dark surfaces · warm gold accents · clean mono type · restrained contrast
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
/* Confidence bar */
.cbar { height: 2px; background: var(--bg3); border-radius: 1px; margin: 10px 0 14px; }
.cbar-fill { height: 100%; border-radius: 1px; background: linear-gradient(90deg, var(--gold2), var(--green)); }
.scard.short .cbar-fill { background: linear-gradient(90deg, var(--gold2), var(--red)); }
.cbar-label {
  display: flex;
  justify-content: space-between;
  font-size: 0.52rem;
...
.lvl-l { font-size: 0.48rem; color: var(--text3); letter-spacing: 0.15em; text-transform: uppercase; }
.lvl-v { font-size: 0.82rem; font-weight: 500; color: var(--text); margin-top: 3px; }
...
.conf-item::before { content: '›'; color: var(--gold); flex-shrink: 0; }
.warn-item {
  font-size: 0.62rem;
  color: #cc9944;
  padding: 3px 0;
...
.warn-item::before { content: '⚠'; flex-shrink: 0; font-size: 0.55rem; }
...
.risk-cell .r-label { font-size: 0.5rem; color: var(--text3); letter-spacing: 0.12em; text-transform: uppercase; }
.risk-cell .r-val   { font-size: 0.85rem; font-weight: 500; color: var(--gold2); margin-top: 2px; }
...
.ov-chg { font-size: 0.7rem; font-weight: 500; margin-left: 6px; }
...
.sr-num  { font-size: 0.62rem; color: var(--text3); }
.sr-name { font-size: 0.8rem; font-weight: 500; color: var(--text); }
.sr-price{ font-size: 0.82rem; font-weight: 500; color: var(--text); text-align: right; }
.sr-chg  { font-size: 0.75rem; font-weight: 500; text-align: right; }
...
.no-sig-icon  { font-size: 2rem; color: var(--text3); margin-bottom: 10px; }
.no-sig-title { font-family: 'Clash Display', sans-serif; font-size: 1rem; color: var(--text2); font-weight: 600; }
.no-sig-sub   { font-size: 0.6rem; color: var(--text3); letter-spacing: 0.15em; margin-top: 6px; }
...
.stMetric label { color: var(--text3) !important; font-size: 0.55rem !important; letter-spacing: 0.15em !important; }
.stMetric [data-testid="stMetricValue"] { color: var(--text) !important; font-size: 1.05rem !important; font-weight: 500 !important; }
...
def build_chart(df: pd.DataFrame, symbol: str, sigs: list[Signal]) -> go.Figure:
    BG    = '#0a0a0a'
    GRID  = '#161616'
    GOLD  = '#c9a84c'
    GREEN = '#3ddc84'
    ...
    if 'ichi_span_a' in df.columns:
        ...
        fig.add_trace(go.Scatter(
            x=df['time'], y=df['ichi_tenkan'],
            line=dict(color='rgba(74,158,255,0.55)', width=0.8),
            name='Tenkan', showlegend=False,
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df['time'], y=df['ichi_kijun'],
            line=dict(color='rgba(240,84,84,0.55)', width=0.8),
            name='Kijun', showlegend=False,
        ), row=1, col=1)

    # ── Bollinger Bands ───────────────────────────────────────────────────────
    for col_name, lc in [('bb_upper', 'rgba(201,168,76,0.3)'), ('bb_lower', 'rgba(201,168,76,0.3)')]:
    ...
    for sig in sigs:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers+text',
            marker=dict(symbol=sym, size=14, color=mc, line=dict(color='#fff', width=1)),
...
def render_signal_card(sym: str, tf: str, sig: Signal, balance: float, risk_pct: float):
    ...
    regime_colors = {
        'TRENDING_UP':   '#3ddc84', 'TRENDING_DOWN': '#f05454',
        'TRENDING':      '#4a9eff', 'RANGING':       '#c9a84c',
        'VOLATILE':      '#cc66ff',
    }
...
<div class="scard {sc}">
  <div class="sc-head">
    <div class="sc-left">
      ...
      <div class="sc-meta">
        ...
        <span class="sc-regime" style="color:{rc};border-color:{rc}33">{sig.regime}</span>
        <span class="sc-regime">{sym} · {tf}</span>
        {tags_h}
      </div>
    </div>
    <div class="sc-right">
  ...
  <div class="lvl-row">
    <div class="lvl entry"><div class="lvl-l">Entry</div><div class="lvl-v">${sig.entry:,.2f}</div></div>
    <div class="lvl sl">   <div class="lvl-l">Stop Loss</div><div class="lvl-v">${sig.sl:,.2f}</div></div>
    <div class="lvl tp1">  <div class="lvl-l">Target 1</div><div class="lvl-v">${sig.tp1:,.2f}</div></div>
    <div class="lvl tp2">  <div class="lvl-l">Target 2</div><div class="lvl-v">${sig.tp2:,.2f}</div></div>
  </div>
  <div class="lvl-row2">
    <div class="lvl tp3">  <div class="lvl-l">Target 3</div><div class="lvl-v">${sig.tp3:,.2f}</div></div>
    <div class="lvl stat"> <div class="lvl-l">SL Dist</div><div class="lvl-v">{sig.sl_pct:.2f}%</div></div>
    <div class="lvl stat"> <div class="lvl-l">TP1 Dist</div><div class="lvl-v">{sig.tp1_pct:.2f}%</div></div>
    <div class="lvl stat"> <div class="lvl-l">ATR</div><div class="lvl-v">${sig.atr:.2f}</div></div>
  </div>
  ...
  <div class="risk-row">
    <div class="risk-cell"><div class="r-label">Risk Amount</div><div class="r-val">${ra:,.2f}</div></div>
    <div class="risk-cell"><div class="r-label">Position (USDT)</div><div class="r-val">${usdt:,.2f}</div></div>
    <div class="risk-cell"><div class="r-label">Contracts</div><div class="r-val">{coins:.6f}</div></div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown(
        '<div style="font-family:\'Clash Display\',sans-serif;font-weight:700;'
        'font-size:1.1rem;color:#ffffff;letter-spacing:-0.01em;margin-bottom:16px">'
        '◈ Terminal Config</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div style="font-size:0.55rem;color:#3ddc84;letter-spacing:0.15em;'
        'margin-bottom:10px">● BINANCE PUBLIC API · NO KEY REQUIRED</div>',
        unsafe_allow_html=True,
    )
...
if not symbols:
    st.markdown(
        '<div class="no-sig"><div class="no-sig-icon">◈</div>'
        '<div class="no-sig-title">No Symbols Selected</div>'
        '<div class="no-sig-sub">Add symbols in the sidebar to begin</div></div>',
        unsafe_allow_html=True,
    )
    st.stop()
...
<div class="ticker-cell">
  ...
  <div class="t-val">${t['price']:,.2f}</div>
  <div class="t-val {cls}" style="font-size:0.68rem;margin-top:1px">{sign}{chg:.2f}%</div>
</div>"""
...
<div class="ticker-cell">
  ...
  <div class="t-val" style="color:{rc_col};font-size:0.75rem">{dom_reg}</div>
</div>
<div class="ticker-cell">
  <div class="t-label">Signals</div>
  <div class="t-val {'up' if total > 0 else ''}">{total}</div>
</div>
<div class="ticker-cell">
...
with tab_mkt:
    st.markdown(
        '<div style="font-size:0.6rem;letter-spacing:0.2em;color:#444;margin-bottom:12px">'
        'REAL-TIME MARKET DATA · BINANCE PUBLIC</div>',
        unsafe_allow_html=True,
    )

    # Stock-list style rows
    st.markdown("""
<div style="background:#111;border:1px solid #2a2a2a;border-radius:5px;overflow:hidden;margin-bottom:16px">
<div class="stock-row" style="background:#181818;border-bottom:1px solid #2a2a2a">
  <div class="sr-num" style="color:#555">#</div>
  <div style="font-size:0.52rem;color:#444;letter-spacing:0.15em">SYMBOL</div>
  <div style="font-size:0.52rem;color:#444;letter-spacing:0.15em;text-align:right">PRICE</div>
  <div style="font-size:0.52rem;color:#444;letter-spacing:0.15em;text-align:right">24H CHG</div>
  <div style="font-size:0.52rem;color:#444;letter-spacing:0.15em;text-align:right">SIGNAL</div>
</div>
""", unsafe_allow_html=True)
...
<div class="stock-row">
  ...
  <div class="sr-name">{short}{regime_badge}</div>
  <div class="sr-price">${t['price']:,.2f}</div>
  <div class="sr-chg" style="color:{chg_c}">{sign}{chg:.2f}%</div>
  <div class="sr-sig">{sig_lbl}</div>
</div>
""", unsafe_allow_html=True)
...
<div class="ov-card">
  ...
  <div>
    <span class="ov-price">${t['price']:,.2f}</span>
    <span class="ov-chg" style="color:{chg_c}">{'+'if chg>=0 else ''}{chg:.2f}%</span>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px">
    <div style="font-size:0.6rem"><span style="color:#444">RSI</span><br>
      <span style="color:{'#f05454' if L['rsi']>70 else ('#3ddc84' if L['rsi']<30 else '#d0d0d0')}">{L['rsi']:.1f}</span></div>
    <div style="font-size:0.6rem"><span style="color:#444">ADX</span><br>
      <span style="color:{'#3ddc84' if L['adx']>25 else '#c9a84c'}">{L['adx']:.1f}</span></div>
    <div style="font-size:0.6rem"><span style="color:#444">RVOL</span><br>
      <span style="color:{'#3ddc84' if L['rvol']>1.5 else '#d0d0d0'}">{L['rvol']:.2f}×</span></div>
    <div style="font-size:0.6rem"><span style="color:#444">Squeeze</span><br>
      <span style="color:{'#c9a84c' if L.get('bb_squeeze',False) else '#333'}">{'ON' if L.get('bb_squeeze',False) else 'OFF'}</span></div>
  </div>
  <div style="margin-top:8px;font-size:0.55rem;color:{rc2};letter-spacing:0.12em">{rg}</div>
  <div style="font-size:0.55rem;color:#444;margin-top:3px">{len(res['signals'])} signal(s) detected</div>
</div>
""", unsafe_allow_html=True)
...
with tab_log:
    ...
    if not log:
        st.markdown(
            ...
            '<div class="no-sig-sub">Signals will appear here after scanning</div></div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f'<div style="font-size:0.55rem;color:#444;margin-bottom:8px">'
            f'{len(log)} signals recorded this session</div>',
            unsafe_allow_html=True,
        )
        st.markdown("""
<div class="log-hdr">
...
<div class="log-row-el">
  <div style="color:#444">{e['ts']}</div>
  <div style="color:#d0d0d0">{e['sym']}</div>
  <div style="color:#666">{e['strat']}</div>
  <div style="color:{dc};font-weight:500">{e['dir']}</div>
  <div style="color:{gc};font-weight:500">{e['grade']} <span style="color:#444;font-size:0.55rem">({e['conf']}%)</span></div>
  <div style="color:#d0d0d0">${e['entry']:,.2f}</div>
  <div style="color:{rc2};font-size:0.58rem">{e.get('regime','?')}</div>
</div>
""", unsafe_allow_html=True)
